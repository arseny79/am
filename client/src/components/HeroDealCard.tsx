...
const DEFAULT_HERO_CARD_EMPTY = {
  curatedLabel: "Curated Opportunity",
  manuallyReviewedLabel: "Manually Reviewed",
  title: "Confidential iGaming Asset",
  category: "iGaming Operator",
  region: "Europe",
  description: "New opportunities are published after manual review. Submit your business or share your acquisition mandate to be notified when live listings become available.",
  shareMandateBtn: "Share Mandate",
  submitBusinessBtn: "Submit a Business",
  viewListingBtn: "View Listing",
  // Static metric/redaction labels used in both empty-state and live card
...
export default function HeroDealCard() {
  ...
  const emptyState = (() => {
    try { return settings?.heroDealCardEmptyJson ? { ...DEFAULT_HERO_CARD_EMPTY, ...JSON.parse(settings.heroDealCardEmptyJson) } : DEFAULT_HERO_CARD_EMPTY; }
    catch (e) { return DEFAULT_HERO_CARD_EMPTY; }
  })();
  const curatedLabel = settings?.heroDealCardCuratedLabel || DEFAULT_HERO_CARD_EMPTY.curatedLabel;
  const manuallyReviewedLabel = settings?.heroDealCardManuallyReviewedLabel || DEFAULT_HERO_CARD_EMPTY.manuallyReviewedLabel;
      ...
      <div className={CARD_SHELL}>
        ...
        <div className="flex items-center justify-between pb-4 mb-4 border-b border-border/60 text-xs font-mono">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[var(--am-accent-lavender)] font-bold uppercase tracking-wide">{emptyState.curatedLabel}</span>
          </div>
          <span className="px-2 py-0.5 rounded bg-primary/15 text-[var(--am-accent-lavender)] border border-primary/25 text-[10px] font-semibold">
            {emptyState.manuallyReviewedLabel}
          </span>
        </div>
        ...
        <div className="mb-4">
          <div className="flex items-center gap-2 flex-wrap mb-1">
            <h3 className="font-bold text-white/70 text-lg tracking-tight">{emptyState.title}</h3>
          ...
          <div className="flex items-center gap-2 text-xs font-mono text-[var(--am-text-dim)]">
            <span>{emptyState.region}</span>
            <span className="text-zinc-700">•</span>
            <span className="text-[var(--am-accent-lavender)]/70">{emptyState.ndaLabel}</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2 p-3 rounded-xl bg-background/60 border border-border/40 mb-5 font-mono flex-1 content-start">
          {[
            { label: emptyState.askingPriceLabel, color: "text-white/30" },
            { label: emptyState.monthlyRevenueLabel, color: "text-emerald-400/30" },
            { label: emptyState.annualRevenueLabel, color: "text-[var(--am-accent-lavender)]/30" },
            { label: emptyState.clientsLabel, color: "text-cyan-400/30" },
          ].map(({ label, color }) => (
            <div key={label} className="p-2 rounded bg-card/40">
              ...
              <span className={`text-sm font-bold ${color}`}>{emptyState.ndaLabel}</span>
            </div>
          ))}
        </div>
        ...
        <div className="pt-4 border-t border-border/40 flex items-center justify-between gap-3 flex-wrap">
          <div>
            <span className="text-[10px] font-mono text-[var(--am-text-dim)]/60 uppercase block">{emptyState.askingPriceLabel}</span>
            <span className="text-xl font-mono font-extrabold text-white/30">{emptyState.confidentialLabel}</span>
          </div>
          <div className="flex gap-2 flex-wrap justify-end">
            <Link href="/buy-asset">
              <Button size="sm" variant="outline" className="text-xs border-border/60">{emptyState.shareMandateBtn}</Button>
            </Link>
            <Link href="/create-listing">
              <Button size="sm" className="text-xs">{emptyState.submitBusinessBtn}</Button>
            </Link>
          </div>
        </div>
      </div>
    ...
    <div className={`${CARD_SHELL} group hover:border-[var(--am-accent-lavender)] transition-all`}>
      <div className="flex items-center justify-between pb-4 mb-4 border-b border-border text-xs font-mono">
        <div className="flex items-center gap-2">
          ...
          <span className="text-emerald-400 font-bold uppercase">
            {isPremium ? "Premium Listing" : curatedLabel}
          </span>
        </div>
        <span className="text-[var(--am-text-dim)] text-[11px]">{manuallyReviewedLabel}</span>
      </div>
        ...
        <div className="flex items-center gap-2 mt-1 text-xs font-mono text-[var(--am-text-dim)]">
          ...
          {listing.confidentialityLevel === "nda" && (
            <>
              ...
              <span className="text-[var(--am-accent-lavender)]">{emptyState.ndaLabel}</span>
            </>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2.5 p-3.5 rounded-xl bg-background border border-border mb-5 font-mono flex-1 content-start">
        <div className="p-2 rounded bg-card/60">
          <span className="text-[10px] uppercase text-[var(--am-text-dim)] block mb-0.5">{emptyState.askingPriceLabel}</span>
          <span className="text-sm font-bold text-white">
            {isConfidential ? emptyState.ndaLabel : formatCurrency(listing.askingPrice)}
          </span>
        </div>
        <div className="p-2 rounded bg-card/60">
          <span className="text-[10px] uppercase text-[var(--am-text-dim)] block mb-0.5">{emptyState.monthlyRevenueLabel}</span>
          <span className="text-sm font-bold text-emerald-400">
            {isConfidential ? emptyState.ndaLabel : formatCurrency(listing.monthlyRecurringRevenue)}
          </span>
        </div>
        <div className="p-2 rounded bg-card/60">
          <span className="text-[10px] uppercase text-[var(--am-text-dim)] block mb-0.5">{emptyState.annualRevenueLabel}</span>
          <span className="text-sm font-bold text-[var(--am-accent-lavender)]">
            {isConfidential ? emptyState.ndaLabel : formatCurrency(listing.annualRevenue)}
          </span>
        </div>
        <div className="p-2 rounded bg-card/60">
          <span className="text-[10px] uppercase text-[var(--am-text-dim)] block mb-0.5">{emptyState.clientsLabel}</span>
          <span className="text-sm font-bold text-cyan-400">
            {isConfidential ? emptyState.ndaLabel : listing.clientCount ?? "N/A"}
          </span>
        </div>
      </div>
      ...
      <div className="pt-4 border-t border-border flex items-center justify-between">
        <div>
          <span className="text-[10px] font-mono text-[var(--am-text-dim)] uppercase block">{emptyState.askingPriceLabel}</span>
          <span className="text-xl font-mono font-extrabold text-white">
            {isConfidential ? emptyState.ndaLabel : formatCurrency(listing.askingPrice)}
          </span>
        </div>
        <Link href={`/listing/${listing.id}`}>
          <Button className="font-mono text-xs font-bold">{emptyState.viewListingBtn}</Button>
        </Link>
      </div>
    </div>
  );
