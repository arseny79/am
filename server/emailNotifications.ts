...
const FROM_EMAIL = process.env.SENDGRID_FROM_EMAIL || "office@acq.market";
const FROM_NAME = process.env.SENDGRID_FROM_NAME || "AM iGaming Marketplace";
const FRONTEND_URL = process.env.VITE_FRONTEND_URL || "https://acq.market";
...
export async function sendEmailVerification(params: {
  ...
  return sendEmail({
    to: params.email,
    subject: "Verify your email address – AM iGaming Marketplace",
    html: `
      <h1>Welcome to AM iGaming Marketplace, ${params.name}!</h1>
...
export async function sendPasswordReset(params: {
  ...
  await sendEmail({
    to: params.email,
    subject: "Reset your password – AM iGaming Marketplace",
    html: `
...
export async function sendCredentialVerifiedEmail(params: {
  ...
  await sendEmail({
    to: params.email,
    subject: "Credential Verified – AM iGaming Marketplace",
    html: `
...
export async function sendCredentialRejectedEmail(params: {
  ...
  await sendEmail({
    to: params.email,
    subject: "Credential Verification Update – AM iGaming Marketplace",
    html: `
