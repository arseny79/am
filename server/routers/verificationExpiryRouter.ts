...
export const verificationExpiryRouter = router({
  ...
  sendExpiryReminder: adminProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      await sendEmail({
        to: user.email,
        subject: `Your Verification Expires in ${daysUntilExpiry} Days - acquisitions.market`,
        html: `
          <!DOCTYPE html>
          <html>
          <head>
...
Renew your verification: ${process.env.VITE_APP_URL || 'https://acquisitions.market'}/verify-account
