...
export const verificationExpiryRouter = router({
  ...
  sendExpiryReminder: adminProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      await sendEmail({
        to: user.email,
        subject: `Your Verification Expires in ${daysUntilExpiry} Days - acquisition.market`,
        html: `
          <!DOCTYPE html>
          <html>
          <head>
...
Renew your verification: ${process.env.VITE_APP_URL || 'https://acquisition.market'}/verify-account
