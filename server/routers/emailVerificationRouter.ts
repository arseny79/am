...
export const emailVerificationRouter = router({
  ...
  sendVerificationEmail: protectedProcedure.mutation(async ({ ctx }) => {
    ...
    const baseUrl = ctx.req?.headers?.origin || process.env.SITE_URL || "https://acq.market";
    const verificationUrl = `${baseUrl}/verify-email?token=${token}`;
    
    await sendEmail({
      to: user.email,
      subject: "Verify Your Email - AM iGaming Marketplace",
      html: `
        <!DOCTYPE html>
        <html>
        <head>
        ...
        <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
          <div style="background-color: #f8f9fa; border-radius: 8px; padding: 30px; margin-bottom: 20px;">
            <h1 style="color: #2563eb; margin-top: 0;">Welcome to AM iGaming Marketplace!</h1>
          ...
          <div style="border-top: 1px solid #e5e7eb; padding-top: 20px; font-size: 12px; color: #666;">
            ...
            <ul style="margin: 10px 0;">
              ...
              <li>Create iGaming listings</li>
              <li>Request access to confidential listings</li>
              <li>Receive important notifications about your deals</li>
            </ul>
            <p style="margin-top: 20px;">
              If you didn't create an account on AM iGaming Marketplace, please ignore this email.
            </p>
          </div>
        </body>
        </html>
