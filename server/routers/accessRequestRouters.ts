...
export const accessRequestRouter = router({
  ...
  create: kycVerifiedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (seller?.email) {
        const reviewUrl = `${process.env.VITE_APP_URL || 'https://acquisitions.market'}/dashboard?tab=access-requests`;
        await sendEmail({
          to: seller.email,
  ...
  respond: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (buyer?.email) {
        const listingUrl = `${process.env.VITE_APP_URL || 'https://acquisitions.market'}/listing/${listing.id}`;
            ...
            html: `
              <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                ...
                <p style="margin: 30px 0;">
                  <a href="${process.env.VITE_APP_URL || 'https://acquisitions.market'}/browse" style="background-color: #2563eb; color: white; padding: 12px 24px; text-decoration: none; border-radius: 6px; display: inline-block;">Browse Listings</a>
                </p>
