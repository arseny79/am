...
export const dealRouter = router({
  ...
  updateStage: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (otherUser?.email) {
        const dealUrl = `${process.env.VITE_APP_URL || 'https://acquisitions.market'}/deal/${deal.id}`;
...
export const messageRouter = router({
  send: verifiedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (recipient?.email) {
        const dealUrl = `${process.env.VITE_APP_URL || 'https://acquisitions.market'}/deal/${dealId}`;
